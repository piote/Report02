...
public class CList <E>{
	...
	public void insert(E newItem) {//last가 가리키는 노드의 다음에 새노드 삽입
		...
		if(last==null) { //리스트가 empty일때
			...
			newNode.setNext(last.getNext());//newNode 의 다음노드가
											//last가 가리키는 노드의 다음노드가 되도록
			last.setNext(newNode);//last가 가리키는 노드의 다음노드가 newNode가 되도록
		}
		size++;
	}	
	public Node delete() {//last가 가리키는 노드의 다음노드 제거
		if(isEmpty())throw new NoSuchElementException();
		Node x = last.getNext();//x가 리스트의 첫노드를 가리킴
	...
	public void print() {//연결리스트 노드들의 항목들을 차레로 출력
		if(size>0) {
			int i = 0;
